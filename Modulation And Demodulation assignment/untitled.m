...
saveas(gcf, 'pamconstellationm2.fig');
...
saveas(gcf, 'pamconstellationm8.fig');
%% Repeat it for QAM
k = 3;
M = 8;
N = [10000*k 1];
...
%% Add AWGN with SNR=20 dB for 64-QAM
k = 3;
M = 64;
N = [10000*k 1];
i_data = randi([0 1], N);
...
saveas(gcf, 'qamreceivedconstellation20db.fig');
%% Add AWGN with SNR=20 dB for 8-PSK
k = 3;
M = 8;
N = [10000*k 1];
i_data = randi([0 1], N);
...
errors = sum(i_data ~= o_data);
fprintf('Number of errors with 8-PSK and SNR=20 dB: %i\n', errors);
figure;
scatterplot(rx_sig);
title("Received Constellation (8-PSK, SNR=20 dB)")
saveas(gcf, 'pskreceivedconstellation20db.fig');
%% Add AWGN with SNR=12 dB for 64-QAM
k = 3;
M = 64;
N = [10000*k 1];
i_data = randi([0 1], N);
...
rx_sig = awgn(tx_sig, SNR);
o_data = pskdemod(rx_sig, M);
fprintf('Number of errors with 8-PSK and SNR=12 dB: %i\n', errors);
figure;
scatterplot(rx_sig);
title("Received Constellation (8-PSK, SNR=12 dB)")
saveas(gcf, 'psksnrreceivedconstellation12db.fig');
